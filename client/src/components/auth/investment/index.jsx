        ...
        {invest?.map(
          (val, index) =>
            step === index && (
              <div>
                <h6>{val?.head}</h6>
               {index!=4? <div className={`${index===1||index===5?'d-flex flex-wrap my-3 gap-3':`row  row-cols-${val?.field!=='investorType'&&2}`} `}>
                  {val?.option?.map((v, i) => (
                    <div className={`${index===1||index===5?"":"mb-4"} `}>
                      <div onClick={()=>handleChange(val?.field,v,val?.isSingle)}
                        className={`bg-border cursor-pointer ${active.includes(v)&&"border"} d-flex align-items-center gap-2 rounded-5 p-3 px-4 text-very-light-gray`}
                        key={index + " " + i}
