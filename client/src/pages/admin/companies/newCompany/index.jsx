...
const handlepdateDoc=(e)=>{
    ...
    setUpdateDoc([...updateDoc,{[name]:files[0] ,id:updateDoc.length,date:new Date().toLocaleDateString()}]);
}
const handleInvestDoc=(e)=>{
    const {name,files}=e.target;
    setInvestDoc([...investDoc,{[name]:files[0],id:investDoc.length,date:new Date().toLocaleDateString()}]);
}
...
const handleSubmit=async(e)=>{
  ...
  return (
     ...
     <div className="container">
            ...
            <div className="col-9 ps-0">
                <div>
                    <div className='cover-profile d-flex flex-column'>
                        <img src={cover||Server+company?.cover} className='position-absolute rounded-3 inset-0 w-100 h-100' alt="cover" />
                        <input onChange={handleChange} name='cover' id='cover' type="file"  />
                        <label htmlFor="cover" className='p-1 px-2 cover-label rounded bg-green text-white'><FaPlus size={20}/></label>
                        <div className="mt-auto">
                        <div className="profile  ">
                            <img src={profile||Server+company?.profile} className='position-absolute w-100 h-100 rounded-circle' style={{inset:"0"}} alt="" />
                            <input name='profile' onChange={handleChange} type="file" id='profile' />
                            <label htmlFor="profile" className='p-1 px-2 pro-label rounded bg-green text-white'><FaPlus size={20}/></label>
                        </div>
                        </div>
                    </div>
                    ...
                    <div className="about col-md-9 d-flex bg-white my-3 py-1  border border-2 rounded px-3 justify-content-between align-items-center">
                        <div className='fw-semibold'>ABOUT {isAbout?<input type="text" name='name' onChange={handleChange} value={company?.name} className='py-2 rounded' placeholder='company name' />:<span className='text-capitalize'>{company?.name}</span>}</div>
                        <div onClick={()=>setisAbout(!isAbout)} style={{width:"35px",aspectRatio:"1/1"}} className='d-flex align-items-center justify-content-center bg-pink text-white cursor-pointer rounded-circle'><FaPencil size={20}/></div>
                    </div>
                    <div>
                        <textarea name="about" value={company?.about} style={{cursor:isAbout?"auto":"not-allowed"}} disabled={!isAbout} onChange={handleChange} placeholder={`${isAbout?"Enter description":"click on edit button."}  `} className='w-100 border bg-white rounded p-3'  rows={4} id=""></textarea>
                    </div>
                </div>
            </div>
            <div className="col-3 h-100 d-flex flex-column">
                <div className="deal d-flex flex-column h-100">
                <div className="about d-flex bg-white my-3 py-1 border border-2 rounded px-2 justify-content-between align-items-center">
                        <div className='fw-semibold'>DEAL SUMMARY</div>
                        <div onClick={()=>setIsDealEdit(!isDealEdit)} style={{width:"35px",aspectRatio:"1/1"}} className='d-flex align-items-center justify-content-center bg-pink text-white cursor-pointer rounded-circle'><FaPencil  size={20}/></div>
                    </div>
                    <div className="deal-list px-4 h-100 py-4 pb-5 rounded bg-white border border-2 d-flex flex-column gap-3 h-100">
                       {data?.map((val,index)=>( <div  key={index}>
                            <label htmlFor={val?.name} className='text-muted'>{val?.label}</label>
                            <div>
                               {isDealEdit? <input name={val?.name} onChange={handleDeal} type={val?.name==="investDate"?"date":"text"} placeholder='-' className=' rounded px-2 w-100'  />:deal&&deal[val?.name]?<span className='fw-semibold'>{deal[val?.name]}</span>:"-"}
                            </div>
                        </div>))}
                    </div>
                </div>
        ...
        <div className="row pb-4">
            ...
            <div className=" d-flex bg-white my-3 py-2 border border-2 rounded px-3 justify-content-between align-items-center">
                        ...
                        <div style={{width:"30px",aspectRatio:"1/1"}} className='d-flex align-items-center justify-content-center bg-green text-white cursor-pointer rounded-2'><FaPlus onClick={()=>setIsNew(!isNews)} size={20}/></div>
            ...
            <div className="col-md-6  row">
                <div className="col-6">
                    <div>   <div className=" position-relative d-flex bg-white my-3 py-2 border border-2 rounded px-3 justify-content-between align-items-center">
                        <div className='fw-semibold'>UPDATE</div>
                        <input type="file" name='updatedoc' onChange={handlepdateDoc} style={{opacity:"0"}} className='position-absolute'  /> 
                        <label htmlFor="updatedoc"> <div style={{width:"30px",aspectRatio:"1/1"}} className='d-flex align-items-center justify-content-center bg-green text-white cursor-pointer rounded-2'><FaPlus  size={20}/></div></label>
                    </div>
                    <div className="details bg-white rounded d-flex flex-column gap-3 py-3 ">
                      {updateDoc.length>0&&updateDoc.map((val,i)=>(
                                    <div className="d-flex  mx-2 ms-3 gap-2">
                                        ...
                                        <div onClick={()=>removeupdateDoc(val?.id)} className='ps-3 text-muted'><FaTrashAlt className='cursor-pointer'  size={20}/></div>
                                    </div>
                      ))}
                      </div></div>
                </div>
                <div className="col-6">
                    <div>   <div className=" position-relative d-flex bg-white my-3 py-2 border border-2 rounded px-3 justify-content-between align-items-center">
                        ...
                        <input type="file" name='updatedoc' onChange={handleInvestDoc} style={{opacity:"0"}} className='position-absolute'  /> 
                        <label htmlFor="updatedoc"> <div style={{width:"30px",aspectRatio:"1/1"}} className='d-flex align-items-center justify-content-center bg-green text-white cursor-pointer rounded-2'><FaPlus  size={20}/></div></label>
                    </div>
                    <div className="details bg-white rounded d-flex flex-column gap-3 py-3 ">
                      {investDoc.length>0&&investDoc.map((val,i)=>(
                                    <div className="d-flex  mx-2 ms-3 gap-2">
                                        <div><FaRegFileLines className='text-muted' size={25}/></div> <div className="d-flex flex-column gap-1">
                                            <small className='text-muted'> CONTRACT UPDATE</small>
                                            <small className='fw-bold'>{val?.date}</small>
                                        </div>
                                        <div onClick={()=>removeInvestDoc(val?.id)} className='ps-3 text-muted'><FaTrashAlt className='cursor-pointer'  size={20}/></div>
                                    </div>
                      ))}
                      </div></div>
                </div>
    ...
    {company&&  <div className="text-center pb-4 "><button className="btn-red w-50">Save</button></div>}
